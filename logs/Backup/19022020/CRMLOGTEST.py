    ...
    for name in files:
        if re.search(r'(dev_..)(.*)', name, re.IGNORECASE) is not None:
            ...
            def get_errors(x):
                ...
                if re.search(r'(\** )[ERROR]{5} (\=>)+', string, re.DOTALL) is not None:
                    ErrorMSG = string
                    if re.search(r'(\** )[WARNING]{7} (\=>)+', string, re.DOTALL) is not None:
                        WarnMSG = string
                    ...
                    a[1] = new_time
                    a[2] = APPServer
                    #a[3] = re.search(r'(\** )[ERROR]{5} (\=>)+', string, re.DOTALL).group(0)
                    a[3] = ErrorMSG
                    a[4] = WarnMSG
                    #a[4] = re.search(r'(WARNING)(\w+)', string, re.DOTALL)
                    #a[5] = WorkProcess
            ...
            #         new_time = old_date.strftime("%X")
            #     #if re.search(r'(ERROR)', string, re.DOTALL) is not None:
            #     if re.search(r'(\** )[WARNING]{5} (\=>)+', string, re.DOTALL) is not None:
            #         WarnMSG = string
            #
            #     #if re.search(r'(WARNING:.*?)(?=ORA-\d+)', string, re.DOTALL) is not None:
            #         b = ["NA", "NA", "NA"]
